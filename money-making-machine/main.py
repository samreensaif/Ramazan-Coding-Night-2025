...
def fetch_side_hustle():
    try:
        ...
        response = requests.get("http://127.0.0.1:8000/side_hustles?apiKey=1234567")
        if response.status_code == 200:  # If request successful
            hustles = response.json()  # Convert response to JSON
            return hustles # Return the hustle idea
        else:
...
def fetch_money_quote():
    try:
        ...
        response = requests.get("http://127.0.0.1:8000/money_quotes?apiKey=12345678")
        if response.status_code == 200:  # If request successful
